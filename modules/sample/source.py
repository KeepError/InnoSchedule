...
def attach_sample_module():
    ...
    @core.bot.message_handler(commands=['set_favorite_string', 'get_favorite_string'])
    def schedule_command_handler(message):
        ...
        if message.text == '/set_favorite_string':
            # send string request
            msg = core.bot.send_message(message.chat.id, permanent.REQUEST_FAVORITE_STRING)
            # register new single-use message handler
            core.bot.register_next_step_handler(msg, process_request_string)
        elif message.text == '/get_favorite_string':
            core.bot.send_message(message.chat.id, favorite_string)
    ...
    def process_request_string(message):
        ...
        core.bot.send_message(message.chat.id, permanent.MESSAGE_STRING_SAVED)
