...
def attach_autoparser_module():
    def get_value(ws: Worksheet, row, col):
        """
    ...
    def parse_new_timetable():
        ...
        # open workbook
        wb = load_workbook(f'{DATABASE_FOLDER}/{permanent.SCHEDULE_NAME}', data_only=True)
        ...
        for start, end in permanent.SCHEDULE_TIMETABLES:
            col = start + 1
            while col <= end:  # here we can replace it with ranges
                course_group = re.split(r"\s+", get_value(ws, 2, col))[0]
                ...
                while row <= permanent.SCHEDULE_LAST_ROW:
                    first_col_value = get_value(ws, row, start)  # time or weekday
                    # extract time
                    ...
                    # if is_english:
                    #     subject_group = subject #list(map(str.strip, re.split(r"\s*/\s*", re.split(r"\s*-\s*", subject)[1])))
